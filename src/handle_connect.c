...
static char *client_id_gen(int *idlen, const char *auto_id_prefix, int auto_id_prefix_len)
{
	...
	*idlen = 36 + auto_id_prefix_len;
	...
	client_id = (char *)calloc((*idlen) + 1, sizeof(char));
	if(!client_id){
		return NULL;
	}
	if(auto_id_prefix){
...
static int will__read(struct mosquitto *context, struct mosquitto_message_all **will, uint8_t will_qos, int will_retain)
{
	...
	will_struct = calloc(1, sizeof(struct mosquitto_message_all));
	if(!will_struct){
		rc = MOSQ_ERR_NOMEM;
		goto error_cleanup;
	}
	...
	if(context->listener->mount_point){
		...
		will_topic_mount = malloc(slen+1);
		if(!will_topic_mount){
			rc = MOSQ_ERR_NOMEM;
			goto error_cleanup;
		}
	...
	if(will_struct->msg.payloadlen > 0){
		will_struct->msg.payload = malloc(will_struct->msg.payloadlen);
...
int handle__connect(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	if(context->listener->use_username_as_clientid){
		if(context->username){
			free(client_id);
			client_id = strdup(context->username);
			if(!client_id){
				rc = MOSQ_ERR_NOMEM;
				goto handle_connect_error;
			}
